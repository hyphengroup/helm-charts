...
######################################################
# Workaround for sidecar xcom image
# This section is added on top of the default airflow_local_settings.py since the xcom sidecar image is not parameterized
# https://github.com/apache/airflow/issues/10605
# https://github.com/apache/airflow/blob/1.10.14/airflow/config_templates/airflow_local_settings.py
# https://github.com/apache/airflow/blob/1.10.14/airflow/kubernetes/pod_generator.py#L67
######################################################

PodDefaults.SIDECAR_CONTAINER = k8s.V1Container(
    name=PodDefaults.SIDECAR_CONTAINER_NAME,
    command=['sh', '-c', PodDefaults.XCOM_CMD],
    image={{ .Values.xcomSidecar.image | quote }},
    volume_mounts=[PodDefaults.VOLUME_MOUNT],
    resources=k8s.V1ResourceRequirements(
        requests={
            "cpu": {{ .Values.xcomSidecar.cpuRequest | quote }},
            "memory": {{ .Values.xcomSidecar.memoryRequest | quote }}
            },
        limits={
            "cpu": {{ .Values.xcomSidecar.cpuLimit | quote }},
            "memory": {{ .Values.xcomSidecar.memoryLimit | quote }}
            }
    ),
)
